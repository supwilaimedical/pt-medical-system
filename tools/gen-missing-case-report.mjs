// gen-missing-case-report.mjs — generate raw_data.caseSummary (narrative Case Report)
// for cases that DON'T have one yet. Same pipeline as regen-case-report.mjs /
// the in-app case-summary logic, but the inverse selection (caseSummary is null).
//   node tools/gen-missing-case-report.mjs --dry   (preview, no DB write)
//   node tools/gen-missing-case-report.mjs         (live)
// Requires Node 18+.
...
// Narrative Case-Report prompt — kept identical to regen-case-report.mjs / CASE_PROMPT
const PROMPT =
...
async function main() {
  ...
  // cases WITHOUT a caseSummary (null or absent)
  const q = SUPABASE_URL + '/rest/v1/cases?select=case_id,patient_info,op_info,triage_level,raw_data&raw_data->>caseSummary=is.null';
  const resp = await fetch(q, { headers: sbHeaders });
  if (!resp.ok) throw new Error('select ' + resp.status + ' ' + (await resp.text()));
  const rows = await resp.json();
  console.log('Found ' + rows.length + ' case(s) without a caseSummary.\n');
