...
description: Working rules and conventions for PT Medical System — Supwilai medical/EMS records app (Transport / Firstaid / Location / GPS / Monitor / Fleet). Use whenever editing v2/* modules, shared/, supabase/, cloudflare/, gas/. Especially helpful for mobile UX refactor (preview-first, breakpoint-isolated, logic-byte-identical), GAS backup mirroring, PowerShell Thai encoding gotcha, and GPS_ENABLED gating.
---

# PT Medical System — Working Rules

ระบบบันทึกผู้ป่วย (EMS) ของ Supwilai. Single HTML per module + Supabase + Cloudinary + CF Workers. รายละเอียดระบบดู `design.md`.
...
`shared/config.js` มี credentials — แก้ต้องระวัง. `cloudflare/wrangler.toml` + CF secrets อยู่นอก repo.

เพิ่ม DEFAULTS keys ใหม่ใน `shared/config.js` ตามรูปแบบเดิม.
