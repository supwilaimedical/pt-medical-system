...
| `NOTIFY_EXT_CHECKLIST_TO` | `Ud2d492b0…` | **ส่งตรงหา admin (pEx) แทนเข้ากลุ่ม** — ตั้งเป็นค่าว่างเพื่อกลับไปเข้ากลุ่ม |

## 💰 LINE นับโควตา "ตามจำนวนผู้รับ" ไม่ใช่ตามจำนวนครั้ง (วัดจริง ก.ค. 2026)

| ส่งแบบ | กินโควตา |
|---|---|
| push หาคนเดียว | **1** |
| push เข้ากลุ่ม | **= จำนวนคนในกลุ่ม** (กลุ่ม "Supwilai แจ้งเตือน" มี 3 คน ⇒ ครั้งละ 3) |
| reply (ตอบคนที่ทักมา) | **ฟรี ไม่นับ** |
| broadcast | = จำนวนเพื่อน OA ทั้งหมด (ตอนนี้ 8) |

หลักฐาน: วันที่มีแต่ PT ส่ง (4, 7, 12, 15, 24 ก.ค.) ยอดที่ LINE นับ = จำนวนครั้งที่ยิง **× 3 เป๊ะทุกวัน**
⇒ **เพิ่มคนเข้ากลุ่มเมื่อไหร่ ค่าใช้จ่ายขึ้นทันทีตามจำนวนคน โดยไม่มีใครรู้ตัว**

มติ Pex 2026-07-31:
- **เช็ครถประจำวัน → ส่งตรงหา admin คนเดียว** (ครั้งละ 1 แทน 3 = ประหยัด 3 เท่าโดยจำนวนครั้งเท่าเดิม)
- **แจ้งลา → คงเดิม** ส่งทั้งเข้ากลุ่มและตรงหา admin เพื่อกันลืม (ยอมจ่ายแพงกว่าเพราะพลาดไม่ได้)
